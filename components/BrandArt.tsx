  ...
  return (
    <svg viewBox="0 0 100 64" className={className} aria-hidden>
      <path
        ...
        fill={light ? "#F4EBDD" : "#FFFFFF"}
      />
      <g className="origin-center animate-blink">
        <circle cx="50" cy="32" r="15" fill="#141115" />
      </g>
    </svg>
  );
}
...
export function BlobEye({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 120 110" className={className} aria-hidden>
      ...
      />
      <ellipse cx="70" cy="52" rx="22" ry="15" fill="#F4EBDD" />
      <circle cx="70" cy="52" r="9" fill="#141115" />
    </svg>
  );
}
...
function eye(cx: number, cy: number, s = 1) {
  return (
    <g transform={`translate(${cx} ${cy}) scale(${s})`}>
      <ellipse cx="0" cy="0" rx="14" ry="9" fill="#F4EBDD" />
      <circle cx="0" cy="0" r="5.5" fill="#141115" />
    </g>
  );
}
...
function renderArt(product: Product, fill: string, c: string) {
  switch (product.art) {
      ...
      return (
        <g>
          {[
            ...
            <g key={i}>
              <ellipse cx={b.x} cy={40 + (38 - b.h)} rx={b.w / 2} ry={b.h / 2} fill="#141115" />
              <rect x={b.x - 4} y={58} width="8" height="6" rx="2" fill={c} />
              <rect x={b.x - 5} y={64} width="10" height="96" rx="5" fill="#F4EBDD" stroke="#141115" strokeWidth="2" />
            </g>
          ))}
          {eye(100, 178, 0.6)}
        </g>
    ...
    case "puff":
      return (
        <g>
          <ellipse cx="100" cy="104" rx="66" ry="56" fill="#F4EBDD" stroke={c} strokeWidth="3" />
    ...
    case "tote":
      return (
        <g>
          <path d="M52 70h96v90a8 8 0 0 1-8 8H60a8 8 0 0 1-8-8Z" fill="#F4EBDD" stroke="#141115" strokeWidth="3" />
    ...
    case "set":
      return (
        <g>
          <rect x="34" y="58" width="132" height="96" rx="8" fill="#F4EBDD" stroke="#141115" strokeWidth="3" />
          <rect x="34" y="58" width="132" height="22" rx="8" fill={c} />
          <ellipse cx="118" cy="118" rx="26" ry="20" fill={c} />
          {eye(124, 118, 0.7)}
          {[58, 72, 86].map((x) => (
            <rect key={x} x={x} y={96} width="6" height="48" rx="3" fill="#141115" />
          ))}
        </g>
      );
    case "sticker":
      return (
        <g>
          <rect x="42" y="46" width="116" height="116" rx="10" fill="#F4EBDD" stroke="#141115" strokeWidth="3" transform="rotate(-6 100 104)" />
